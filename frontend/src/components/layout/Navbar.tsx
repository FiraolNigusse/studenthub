  ...
  return (
    <nav className="fixed top-0 left-0 right-0 h-20 bg-white/80 backdrop-blur-xl border-b border-slate-100 z-[1000] flex items-center transition-all isolate overflow-visible">
      <div className="container mx-auto px-6 h-full flex items-center justify-between">
        ...
        {/* Desktop Links */}
        <ul className="hidden lg:flex items-center gap-1.5 p-1 bg-slate-50 border border-slate-100 rounded-2xl shadow-sm relative isolate pointer-events-auto">
          {navLinks.map((link) => (
            <li key={link.path}>
              <Link 
                to={link.path} 
                className={cn(
                  'flex items-center gap-2 px-5 py-2.5 rounded-xl text-[0.95rem] font-bold transition-all relative group isolate',
        ...
        <button 
          ...
          className="lg:hidden p-3 rounded-2xl bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-900 transition-all active:scale-90"
        >
      ...
      {/* Mobile menu overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-20 bg-white/95 backdrop-blur-2xl z-[900] lg:hidden animate-fade-in isolate overflow-y-auto">
          <div className="container mx-auto px-6 py-12 flex flex-col gap-4">
             {navLinks.map((link) => (
                <Link 
                  ...
                  className={cn(
                    'flex items-center gap-4 p-6 rounded-3xl text-xl font-display font-extrabold transition-all border-2',
                    location.pathname === link.path 
                      ? 'bg-primary-50 text-primary-600 border-primary-200 shadow-xl shadow-primary-500/10' 
